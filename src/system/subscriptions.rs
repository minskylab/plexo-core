...
impl SubscriptionManager {
    ...
    pub async fn add_subscription(&self, sender: Sender<DataContainer>, option:i32) -> MyResult<String> {
        ...
        if option == 1
        {
            ...
            subscriptions.insert(self.id_project.clone(), Subscription::new(self.id_project.clone(), sender));
        ...
        else
         {
                ...
                subscriptions.insert(self.id_team.clone(), Subscription::new(self.id_team.clone(), sender));
    ...
    pub async fn send_task_event(&self, event: Task) -> MyResult<Task> {
        ...
        if let Some(subscription) = subscriptions.get_mut(&self.id_task) {
            subscription.sender.clone().try_send(DataContainer::TaskContainer(event.clone())).expect("Fallo al enviar el evento");
        }
        Ok(event)
    }
    ...
    pub async fn send_project_event(&self, event: Project) -> MyResult<Project> {
        ...
        if let Some(subscription) = subscriptions.get_mut(&self.id_project) {
            subscription.sender.clone().try_send(DataContainer::ProjectContainer(event.clone())).expect("Fallo al enviar el evento");
        }
    ...
    pub async fn send_team_event(&self, event: Team) -> MyResult<Team> {
        ...
        if let Some(subscription) = subscriptions.get_mut(&self.id_team) {
            subscription.sender.clone().try_send(DataContainer::TeamContainer(event.clone())).expect("Fallo al enviar el evento");
        }

...
use async_graphql::{
    async_stream::stream, futures_util::StreamExt, Context, FieldResult, Subscription,
};
use chrono::Utc;
use std::pin::Pin;
...
impl SubscriptionRoot {
    async fn subscribe_task(
        &self,
        ctx: &Context<'_>,
    ) -> FieldResult<Pin<Box<dyn Stream<Item = Option<Task>> + Send>>> {
        let (sender, mut receiver) = channel(100);
        let subscription_manager = &ctx.data::<Engine>().unwrap().subscription_manager;
        let new_uuid = Uuid::new_v4().to_string();

        let subscription_added = subscription_manager.add_subscription(sender, 1).await?;
        if subscription_added == new_uuid.clone() {
    ...
    async fn subscribe_project(
        &self,
        ctx: &Context<'_>,
    ) -> FieldResult<Pin<Box<dyn Stream<Item = Option<Project>> + Send>>> {
        let (sender, mut receiver) = channel(100);
        let subscription_manager = &ctx.data::<Engine>().unwrap().subscription_manager;
        let new_uuid = Uuid::new_v4().to_string();

        let subscription_added = subscription_manager.add_subscription(sender, 2).await?;
        if subscription_added == new_uuid.clone() {
    ...
    async fn subscribe_team(
        &self,
        ctx: &Context<'_>,
    ) -> FieldResult<Pin<Box<dyn Stream<Item = Option<Team>> + Send>>> {
        let (sender, mut receiver) = channel(100);
        let subscription_manager = &ctx.data::<Engine>().unwrap().subscription_manager;
        let new_uuid = Uuid::new_v4().to_string();

        let subscription_added = subscription_manager.add_subscription(sender, 3).await?;
        if subscription_added == new_uuid.clone() {
